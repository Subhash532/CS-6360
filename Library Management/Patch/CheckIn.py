...
class CheckIn:
    ...
    def search_book_loans(self):
        ...
        cursor = cnx.cursor()
        cursor.execute("select BOOK_LOANS.Loan_Id, BOOK_LOANS.ISBN, BOOK_LOANS.Card_id, BOOK.title, BOOK_LOANS.Date_in "
                       "from BOOK_LOANS join BORROWER on BOOK_LOANS.Card_id = BORROWER.Card_id "
                       "join BOOK on BOOK_LOANS.ISBN = BOOK.ISBN where BORROWER.Bname like "+search_b)
